...
# ==============================================================================
# FIGURE 1: SYSTEM ARCHITECTURE PIPELINE (7-STAGE FORMAL FLOW)
# ==============================================================================
def generate_fig1():
    ...
    title_box = patches.FancyBboxPatch((0.2, 6.70), 16.1, 0.65, boxstyle='round,pad=0.02,rounding_size=0.03',
                                       facecolor='#1A365D', edgecolor='#0F294A', linewidth=1.5)
    ax.add_patch(title_box)
    ax.text(8.25, 7.03, 'END-TO-END SYSTEM ARCHITECTURE & EVALUATION PIPELINE', 
            ha='center', va='center', fontsize=12.5, fontweight='bold', color='#FFFFFF')

    stages = [
        {
            'num': '1', 'title': 'DATA INGESTION\n& FREEZING',
            'color': '#EBF8FF', 'border': '#3182CE', 'header_bg': '#2B6CB0',
            'boxes': [
        ...
        {
            'num': '2', 'title': 'LEAKAGE AUDIT\n& INTEGRITY',
            'color': '#E6FFFA', 'border': '#319795', 'header_bg': '#285E61',
            'boxes': [
                ('Missing Data Protocol', ['Strict Forward-Fill ($P_t = P_{t-1}$)', 'Holiday Alignments', 'No Backward Interpolation']),
                ('Decoupled Scaling', ['StandardScaler Fitted on Train', 'Zero Validation Contamination', 'Zero Test Information Leak']),
                ('Chronological Order', ['Temporal Order Preserved', 'Strict Causal Structure', 'ADF Unit-Root Check'])
            ]
        },
        {
            'num': '3', 'title': 'FEATURE & TARGET\nENGINEERING',
            'color': '#FAF5FF', 'border': '#805AD5', 'header_bg': '#553C9A',
            'boxes': [
                ...
                ('Sliding Window', ['Look-back Length: $L=30$', 'Batch Tensor: $X \\in \\mathbb{R}^{B \\times L \\times D}$', 'Context: $[\\mu_X, \\sigma_X]$']),
                ('Cumulative Target', ['Log-Return Formulation:', '$R_{t \\to t+h} = \\ln(P_{t+h}/P_t)$', '$h \\in \\{1, 3, 5, 7, 10, 20, 60\\}$'])
            ]
        },
        {
            'num': '4', 'title': 'EVALUATION\nPROTOCOLS',
            'color': '#FEFCBF', 'border': '#D69E2E', 'header_bg': '#975A16',
            'boxes': [
                ('Expanding Window', ['Walk-Forward Protocol', 'Simulated Daily Retraining', 'Real-world Deployment Mode']),
        ...
        {
            'num': '5', 'title': 'UNIFIED MODEL\nTRAINING',
            'color': '#FFF5F5', 'border': '#E53E3E', 'header_bg': '#9B2C2C',
            'boxes': [
                ('GUMNetHet Core', ['3 Heterogeneous Experts', 'Dynamic Context Router', 'Residual Quantile Heads']),
                ('Benchmark Suite', ['Linear: DLinear, LTSF', 'Transformers: PatchTST, iTrans', 'SSM: BiMamba, MambaFormer', 'Foundation: Chronos, TimesFM']),
                ('Loss Optimization', ['Pinball Quantile Loss ($q \\in \\mathcal{Q}$)', 'Load Balance Regularization', 'AdamW + Plateau Decay'])
            ]
        },
        {
            'num': '6', 'title': 'EVALUATION\nDATABASE',
            'color': '#F0FFF4', 'border': '#38A169', 'header_bg': '#22543D',
            'boxes': [
                ('Unified Single Store', ['Full Output JSON Repository', 'Predictions: $\\hat{R}_{t \\to t+h}, \\hat{P}_{t+h}$', 'Residual Error Arrays']),
                ('Multi-Metric Storage', ['Point: MAE, RMSE, MAPE, $R^2$', 'Direction: DA (\\%)', 'Tail: CRPS, PICP, PINAW']),
                ('Parity & Traceability', ['5 Seeds $\\times$ 4 Protocols', '47 Models $\\times$ 7 Horizons', '2 Fuel Target Datasets'])
            ]
        },
        {
            'num': '7', 'title': 'STATISTICAL TESTS\n& DEPLOYMENT',
            'color': '#EDF2F7', 'border': '#4A5568', 'header_bg': '#2D3748',
            'boxes': [
                ...
                ('Deployment Readiness', ['Inference Latency: 1.42 ms', 'Lightweight VRAM: 420 MB', 'Commercial Hedging Value'])
            ]
        }
    ]
    ...
    for i, s in enumerate(stages):
        ...
        col_box = patches.FancyBboxPatch((x, 0.15), col_w, 6.35, boxstyle='round,pad=0.02,rounding_size=0.03',
                                         facecolor=s['color'], edgecolor=s['border'], linewidth=1.2)
        ax.add_patch(col_box)
        
        header_box = patches.FancyBboxPatch((x + 0.04, 5.65), col_w - 0.08, 0.75, boxstyle='round,pad=0.02,rounding_size=0.02',
                                            facecolor=s['header_bg'], edgecolor=s['header_bg'], linewidth=1.0)
        ax.add_patch(header_box)
        ax.text(x + col_w/2, 6.02, f"STAGE {s['num']}\n{s['title']}", 
                ha='center', va='center', fontsize=8.8, fontweight='bold', color='#FFFFFF', linespacing=1.1)

        box_y_starts = [3.90, 2.10, 0.30]
        box_h = 1.65
        for b_idx, (b_title, b_items) in enumerate(s['boxes']):
            by = box_y_starts[b_idx]
            sub_box = patches.FancyBboxPatch((x + 0.08, by), col_w - 0.16, box_h, 
                                            boxstyle='round,pad=0.02,rounding_size=0.02',
                                            facecolor='#FFFFFF', edgecolor=s['border'], linewidth=0.9)
            ax.add_patch(sub_box)
            
            ax.text(x + col_w/2, by + box_h - 0.18, b_title, ha='center', va='center',
                    fontsize=8.2, fontweight='bold', color=s['header_bg'])
            
            for it_idx, item in enumerate(b_items):
                ax.text(x + 0.14, by + box_h - 0.46 - it_idx * 0.37, f"• {item}", ha='left', va='center',
                        fontsize=7.5, color='#2D3748')

        if i < len(stages) - 1:
            ax.annotate('', xy=(x + col_w + col_gap - 0.01, 3.25), xytext=(x + col_w + 0.01, 3.25),
                        arrowprops=dict(facecolor='#4A5568', edgecolor='#4A5568', width=1.8, headwidth=6, headlength=6))

    plt.tight_layout()
    fig.savefig('paper_figures/fig1_system_framework.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
    print("✓ Successfully generated fig1_system_framework.png")


# ==============================================================================
# FIGURE 2: ENLARGED, HIGH-LEGIBILITY GUMNetHet NEURAL ARCHITECTURE
# Larger fonts, wider cards, zero overflow, fully inside canvas
# ==============================================================================
def generate_fig2():
    ...
    ax.add_patch(in_box)
    ax.text(1.70, 8.85, 'Input Sequence Tensor', ha='center', va='center', fontsize=12, fontweight='bold', color='#1A202C')
    ax.text(1.70, 8.50, r'$X \in \mathbb{R}^{B \times L \times D}$ ($L=30$)', ha='center', va='center', fontsize=11.5, fontweight='bold', color='#2B6CB0')
    ...
    ax.add_patch(p1)
    ax.text(1.70, 8.00, 'Spot Price Features', ha='center', va='center', fontsize=11, fontweight='bold', color='#2B6CB0')
    ax.text(1.70, 7.60, r'$x^{\mathrm{CNN}} \in \mathbb{R}^{B \times L \times D_1}$', ha='center', va='center', fontsize=10.5, fontweight='bold', color='#1A365D')
    ax.text(1.70, 7.15, '• MG95, MG92, MG97\n• DO 0.001%, DO 0.05%\n• WTI, Brent, Naphtha', ha='center', va='center', fontsize=9.2, color='#2D3748', linespacing=1.2)
    ...
    ax.add_patch(p2)
    ax.text(1.70, 6.35, 'Macro & GPR Risk', ha='center', va='center', fontsize=11, fontweight='bold', color='#6B46C1')
    ax.text(1.70, 5.95, r'$x^{\mathrm{GRU}} \in \mathbb{R}^{B \times L \times D_2}$', ha='center', va='center', fontsize=10.5, fontweight='bold', color='#44337A')
    ax.text(1.70, 5.50, '• Geopolitical Risk (GPR)\n• USD Index (DXY)\n• GPR_MA30, DXY_MA30', ha='center', va='center', fontsize=9.2, color='#2D3748', linespacing=1.2)
    ...
    ax.add_patch(p3)
    ax.text(1.70, 4.65, 'Ratios & Volatility', ha='center', va='center', fontsize=11, fontweight='bold', color='#C53030')
    ax.text(1.70, 4.25, r'$x^{\mathrm{KAN}} \in \mathbb{R}^{B \times L \times D_3}$', ha='center', va='center', fontsize=10.5, fontweight='bold', color='#742A2A')
    ax.text(1.70, 3.88, '• Crack Spread Ratios\n• Realized Vol (10d, 30d)\n• Day_sin, Day_cos', ha='center', va='center', fontsize=9.2, color='#2D3748', linespacing=1.2)
    ...
    ax.text(5.20, 8.85, 'Expert 1: Multi-Scale 1D-CNN', ha='center', va='center', fontsize=12, fontweight='bold', color='#2B6CB0')
    exp1_text = (
    ...
    ax.text(5.20, 7.95, exp1_text, ha='center', va='center', fontsize=10.5, color='#1A202C', linespacing=1.3)
    ...
    ax.text(5.20, 6.75, 'Expert 2: Stacked GRU-Attention', ha='center', va='center', fontsize=12, fontweight='bold', color='#6B46C1')
    exp2_text = (
    ...
    ax.text(5.20, 5.85, exp2_text, ha='center', va='center', fontsize=10.5, color='#1A202C', linespacing=1.3)
    ...
    ax.text(5.20, 4.75, 'Expert 3: Wavelet-KAN Shock Block', ha='center', va='center', fontsize=12, fontweight='bold', color='#C53030')
    exp3_text = (
    ...
    ax.text(5.20, 4.00, exp3_text, ha='center', va='center', fontsize=10.5, color='#1A202C', linespacing=1.3)
    ...
    ax.text(9.77, 8.85, 'Step 2: Horizon-Aware Dynamic Router', ha='center', va='center',
            fontsize=12.5, fontweight='bold', color='#744210')
    ...
    ax.text(8.32, 8.10, 'Feature Concat', ha='center', va='center', fontsize=11, fontweight='bold', color='#744210')
    cat_content = (
    ...
    ax.text(8.32, 5.75, cat_content, ha='center', va='center', fontsize=10.5, fontweight='bold', color='#2D3748')
    ...
    ax.text(9.92, 7.20, 'MLP Gate', ha='center', va='center', fontsize=11.5, fontweight='bold', color='#744210')
    ax.text(9.92, 6.00, '(128 units)\n+\nGELU\n+\nSoftmax\nLayer', ha='center', va='center', fontsize=10, fontweight='bold', color='#744210', linespacing=1.25)

    # Weights & Fusion Box
    fuse_box = patches.FancyBboxPatch((10.75, 3.65), 1.25, 4.80, boxstyle='round,pad=0.02,rounding_size=0.02',
                                      facecolor='#FFFFFF', edgecolor='#D69E2E', linewidth=1.2)
    ax.add_patch(fuse_box)
    ax.text(11.37, 8.10, 'Router Weights', ha='center', va='center', fontsize=11, fontweight='bold', color='#744210')
    fuse_content = (
        r'$w_1$ (CNN)' + '\n' +
        r'$w_2$ (GRU)' + '\n' +
        r'$w_3$ (KAN)' + '\n\n' +
    ...
    ax.text(11.37, 5.70, fuse_content, ha='center', va='center', fontsize=10, fontweight='bold', color='#2D3748', linespacing=1.25)
    ...
    ax.text(15.30, 8.85, 'Step 3: Horizon Heads & Residual Scaling', ha='center', va='center',
            fontsize=12.5, fontweight='bold', color='#234E52')
    ...
    ax.add_patch(h1)
    ax.text(13.95, 8.15, 'Short Head: $h \\in \\{1, 3\\}$', ha='center', va='center', fontsize=11, fontweight='bold', color='#285E61')
    ax.text(13.95, 7.55, '• Direct Linear Projection\n• Preserves Short Momentum\n• Fast O(1) Latency Execution', 
            ha='center', va='center', fontsize=9.2, color='#2D3748', linespacing=1.2)

    h2 = patches.FancyBboxPatch((12.70, 5.35), 2.50, 1.50, boxstyle='round,pad=0.01,rounding_size=0.02',
                                facecolor='#FFFFFF', edgecolor='#319795', linewidth=1.1)
    ax.add_patch(h2)
    ax.text(13.95, 6.55, 'Medium Head: $h \\in \\{5, 7, 10\\}$', ha='center', va='center', fontsize=11, fontweight='bold', color='#285E61')
    ...
    h3 = patches.FancyBboxPatch((12.70, 3.65), 2.50, 1.50, boxstyle='round,pad=0.01,rounding_size=0.02',
                                facecolor='#FFFFFF', edgecolor='#319795', linewidth=1.1)
    ax.add_patch(h3)
    ax.text(13.95, 4.85, 'Long Head: $h \\in \\{20, 60\\}$', ha='center', va='center', fontsize=11, fontweight='bold', color='#285E61')
    ...
    ax.text(16.65, 8.10, 'Calibrated Tail Risk', ha='center', va='center', fontsize=11.5, fontweight='bold', color='#234E52')
    res_content = (
    ...
    ax.text(16.65, 5.75, res_content, ha='center', va='center', fontsize=9.8, fontweight='bold', color='#2D3748', linespacing=1.25)

    # Connecting Arrows
    arr = dict(facecolor='#4A5568', edgecolor='#4A5568', width=1.8, headwidth=7, headlength=7)
    ...
    # Height = 2.65 inches, wraps tightly around the text without empty void!
    # ==========================================================================
    ...
    ax.add_patch(tab1)
    ax.text(b1_x + b_w/2, 2.24, 'PatchTST (Transformer)', ha='center', va='center', fontsize=11, fontweight='bold', color='#2B6CB0')
    ...
    ax.text(b1_x + b_w/2, 1.20, b1_text, ha='center', va='center', fontsize=9.8, color='#2D3748', linespacing=1.32)
    ...
    ax.add_patch(tab2)
    ax.text(b2_x + b_w/2, 2.24, 'DLinear (Decomposition)', ha='center', va='center', fontsize=11, fontweight='bold', color='#C05621')
    ...
    ax.text(b2_x + b_w/2, 1.20, b2_text, ha='center', va='center', fontsize=9.8, color='#2D3748', linespacing=1.32)
    ...
    ax.add_patch(tab3)
    ax.text(b3_x + b_w/2, 2.24, 'LSTM / GRU-Attention', ha='center', va='center', fontsize=11, fontweight='bold', color='#6B46C1')
    ...
    ax.text(b3_x + b_w/2, 1.20, b3_text, ha='center', va='center', fontsize=9.8, color='#2D3748', linespacing=1.32)
    ...
    ax.add_patch(tab4)
    ax.text(b4_x + b_w/2, 2.24, 'XGBoost MultiOutput', ha='center', va='center', fontsize=11, fontweight='bold', color='#276749')
    ...
    ax.text(b4_x + b_w/2, 1.20, b4_text, ha='center', va='center', fontsize=9.8, color='#2D3748', linespacing=1.32)

    plt.tight_layout()
    fig.savefig('paper_figures/fig2_gumnethet_architecture.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
    print("✓ Successfully generated compact-panel-B fig2_gumnethet_architecture.png")
