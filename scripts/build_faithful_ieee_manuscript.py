...
from scripts.table_builder_helper import M, create_inline_omml
...
def sanitize_data_table(tbl_element, num_cols, font_sz_pt=8.0):
    tbl = deepcopy(tbl_element)
    total_w = 5040 # Exactly 3.5 inches fitting 1 column
    sz_val = "16" # Exactly 8.0pt font size
    
    if num_cols == 7: # Table 1, Table 3, Table 4
        col_widths = [560, 1180, 660, 660, 660, 660, 660]
    elif num_cols == 5: # Table 2, Table 5
        col_widths = [1400, 910, 910, 910, 910]
    else:
        col_w = int(total_w / num_cols)
        remainder = total_w - (col_w * num_cols)
    ...
    if tblPr:
        ...
        tp.append(parse_xml(f'<w:tblBorders {nsdecls("w")}>'
                            ...
                            f'</w:tblBorders>'))
        tp.append(parse_xml(f'<w:tblCellMar {nsdecls("w")}>'
                            f'<w:top w:w="25" w:type="dxa"/>'
                            f'<w:left w:w="20" w:type="dxa"/>'
                            f'<w:bottom w:w="25" w:type="dxa"/>'
                            f'<w:right w:w="20" w:type="dxa"/>'
                            f'</w:tblCellMar>'))
        ...
        for c_idx, cell in enumerate(cells):
            if c_idx < len(col_widths):
                ...
                for p in cell.xpath('./w:p'):
                    ...
                    for jc in pPr.xpath('./w:jc'):
                        pPr.remove(jc)
                    align_val = "left" if (c_idx == 1 and num_cols >= 7) or (c_idx == 0 and num_cols == 5) else "center"
                    if r_idx == 0:
                        align_val = "center"
                    pPr.append(parse_xml(f'<w:jc {nsdecls("w")} w:val="{align_val}"/>'))
...
def build_faithful_ieee_manuscript(is_en=True, out_path="GUMNETHet_FAIRv4_final.docx"):
    print(f"\n========================================================")
    print(f"Building STRICT IEEE TEMPLATE Manuscript ({'EN' if is_en else 'VI'}) -> {out_path}")
    print(f"========================================================")
    
    tmpl_strict = 'conference-template-a4 (1).docx'
    tmpl_trans = 'conference-template-a4_transitional.docx'
    tmpl_clean = 'conference-template-a4_clean_styles.docx'
    
    convert_strict_to_transitional(tmpl_strict, tmpl_trans)
    
    with zipfile.ZipFile(tmpl_trans, 'r') as z_in:
        with zipfile.ZipFile(tmpl_clean, 'w', zipfile.ZIP_DEFLATED) as z_out:
            for item in z_in.infolist():
                data = z_in.read(item.filename)
    ...
    # Base Section 0 (1-Column for Title & Authors)
    ...
    def insert_table_here(table_xml_element):
        p_dummy = doc.add_paragraph()
        p_dummy._p.addnext(deepcopy(table_xml_element))
        body_elem.remove(p_dummy._p)

    # 1. Title (style: paper title)
    p_title = add_p(style_name='paper title', align=WD_ALIGN_PARAGRAPH.CENTER, space_before=12, space_after=12, line_spacing=1.15)
    ...
    # 2. Authors Table (1-row 3-columns, perfectly aligned)
    ...
    authors_data = [
        {
            ...
            "org": "Ho Chi Minh City University of\nTechnology (HUTECH)" if is_en else "Trường Đại học Công nghệ\nTP.HCM (HUTECH)",
    ...
    for col_idx, auth in enumerate(authors_data):
        ...
        r_num = p_c.add_run(auth["num"])
        r_num.font.name = "Times New Roman"
        r_num.font.size = Pt(10.0)
        r_num.font.bold = True
        
        r_sup = p_c.add_run(auth["sup"])
        r_sup.font.name = "Times New Roman"
        r_sup.font.size = Pt(10.0)
        r_sup.font.bold = True
        r_sup.font.superscript = True
        
        r_name = p_c.add_run(" " + auth["name"] + "\n")
        r_name.font.name = "Times New Roman"
        r_name.font.size = Pt(10.0)
        r_name.font.bold = True
        
        r_dept = p_c.add_run(auth["dept"] + "\n")
        r_dept.font.name = "Times New Roman"
        r_dept.font.size = Pt(9.0)
        r_dept.font.italic = True
        
        r_org = p_c.add_run(auth["org"] + "\n")
        r_org.font.name = "Times New Roman"
        r_org.font.size = Pt(9.0)
        r_org.font.italic = True
        
        r_city = p_c.add_run(auth["city"] + "\n")
        r_city.font.name = "Times New Roman"
        r_city.font.size = Pt(9.0)
        
        r_mail = p_c.add_run(auth["email"])
        r_mail.font.name = "Times New Roman"
        r_mail.font.size = Pt(9.0)

    # Transition break to 2 columns body (THE ONLY SECTION BREAK IN THE ENTIRE TEMPLATE!)
    p_trans = add_p(style_name='Normal', align=WD_ALIGN_PARAGRAPH.CENTER, space_before=0, space_after=6)
    sect1_xml = (
        f'<w:sectPr {nsdecls("w")}>'
    ...
    if is_en:
        add_styled_text(p, [
            "The Vietnamese refined petroleum market possesses a distinctive strategic supply structure: approximately 70% of total domestic "
            "consumption is supplied by two domestic oil refineries (Dung Quat and Nghi Son), while the remaining 30% must be imported directly "
            "from international markets. Among these, Singapore serves as the most critical import hub, where Mean of Platts Singapore (MOPS) "
            "refined product trading prices—exemplified by Mogas 95 (MG95) and Gasoil 0.001%S (DO 0.001%)—act as the foundational pricing benchmark "
            "for commercial contracts, cost accounting, and supply chain management. However, Platts prices fluctuate with extreme complexity "
            "due to simultaneous responses to global supply–demand shocks, foreign exchange volatility, and geopolitical risks (GPR) [1], [2], [3]. "
            "Historical episodes such as the 2014–2016 oil price collapse, the 2020 OPEC+ price war, the 2022 Russia–Ukraine conflict, and the "
            "2023–2024 Red Sea tensions demonstrate that refined product price series frequently exhibit non-linear jumps, regime shifts, and severe "
            "volatility clustering. Consequently, accurately forecasting Platts refined petroleum prices over short-term (H1–H7) and medium-term "
            "(H10–H60) horizons is an urgent operational necessity directly serving business decision-making, procurement planning, inventory "
            "management, and risk hedging for major petroleum trading enterprises such as Petrolimex and PVOIL."
        ])
    else:
        add_styled_text(p, [
    ...
    if is_en:
        add_styled_text(p, [
            "Modern time series architectures such as PatchTST [8], iTransformer [9], TimesNet [10], DLinear [11], Mamba [15], and Chronos [16] "
            "have substantially improved performance on general benchmarks; however, most process all input variables within a relatively "
            "homogeneous representation space. For energy data, three signal groups possess fundamentally different natures: high-frequency "
            "price momentum, slowly-evolving macroeconomic regimes, and large-amplitude non-linear shock responses that demand specialized "
            "architectural inductive biases. Traditional mixture-of-experts (MoE) networks [20], [21], [22] typically feed the identical feature set "
            "to all experts, thereby degrading the degree of expert specialization."
        ])
    else:
        add_styled_text(p, [
    ...
    if is_en:
        add_styled_text(p, [
            "The proposed GUMNetHet model resolves this bottleneck via feature partitioning: price and benchmark series are processed by "
            "multi-scale 1D-CNN; macroeconomic and GPR indices by GRU-Attention; and crack-spread ratios and realized volatility by Wavelet-KAN. "
            "These three representations are flexibly aggregated via a gating router conditioned on the forecasting horizon and market context. "
            "The principal contributions of this paper are fourfold: (i) A heterogeneous MoE architecture with feature partitioning grounded "
            "in economic principles and frequency domains; (ii) A horizon-aware routing mechanism (horizon-aware routing); (iii) A multi-quantile "
            "output head (multi-quantile head) combined with residual scaling to mitigate variance drift; and (iv) Extensive expanding walk-forward "
            "empirical evaluations on ", ('m', 'N_val'), " trading days directly within high geopolitical volatility regimes (test set volatility "
            "1.9–2.9× training levels) with comprehensive reporting across MAE, RMSE, MAPE, ", ('m', 'R2'), ", and DA% metrics."
        ])
    else:
        add_styled_text(p, [
    ...
    if is_en:
        add_styled_text(p, [
            "Kilian [2] and Baumeister & Kilian [3] established foundational frameworks for decomposing supply–demand shocks in oil markets, "
            "while the GPR index of Caldara & Iacoviello [1] provides a quantitative metric for geopolitical risk. On the modeling side, "
            "PatchTST [8], iTransformer [9], and TimesNet [10] represent modern Transformer and temporal variation designs; DLinear [11] "
            "demonstrates that decomposition-based linear models remain strong baselines. TFT [12], N-BEATS [13], and N-HiTS [14] extend "
            "multi-horizon forecasting; Mamba [15] introduces selective state-space mechanisms into sequence modeling; and Chronos [16], "
            "TimesFM [17], MOIRAI [18], and TTM [19] expand into foundation time-series architectures."
        ])
    else:
        add_styled_text(p, [
    ...
    if is_en:
        add_styled_text(p, [
            "Regarding MoE, Jacobs et al. [20] introduced adaptive mixtures of local experts; Shazeer et al. [21] and Switch Transformers [22] "
            "advanced sparse gating at large scale. KAN [23] and Wav-KAN [24] replace fixed activation functions with learnable basis functions, "
            "where wavelets are uniquely suited for multi-resolution localized signals [25]. Time-MoE [26] and TimeMixer++ [27] further "
            "demonstrate the value of expert routing in time series. Distinct from homogeneous expert designs, GUMNetHet assigns differentiated "
            "neural architectures to distinct feature groups and explicitly conditions the router on the forecasting horizon."
        ])
    else:
        add_styled_text(p, [
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[0]._tbl))
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[1]._tbl))

    # --- SECTION III-B: OVERALL SYSTEM ARCHITECTURE ---
    ...
    # --- FIGURE 1 (High-DPI Large Font +4 Diagram, Single Column Width) ---
    p_img1 = add_p(align=WD_ALIGN_PARAGRAPH.CENTER, space_before=4, space_after=2)
    p_img1.add_run().add_picture('scratch/images/image1.png', width=Inches(3.48))
    set_cap("Fig. 1." if is_en else "Hình 1.", 
            "Architectural overview of GUMNetHet: (A) Feature partitioning, three heterogeneous experts, horizon-aware router, and multi-quantile head; (B) Paradigms of competitive baselines." if is_en else
            "Tổng quan kiến trúc GUMNetHet: (A) Phân vùng đặc trưng, 3 chuyên gia dị thể, router nhận biết horizon và đầu ra đa phân vị; (B) Các cấu trúc baseline cạnh tranh.", 
            1, is_table=False)
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[2]._tbl))
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[3]._tbl))
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[4]._tbl))
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[5]._tbl))
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[6]._tbl))
    ...
    insert_table_here(sanitize_equation_table(src_doc.tables[7]._tbl))
    ...
    set_cap("TABLE I." if is_en else "Bảng 1.",
            ...
            1, is_table=True)
    insert_table_here(sanitize_data_table(src_doc.tables[8]._tbl, 7, font_sz_pt=8.0))
    ...
    set_cap("TABLE II." if is_en else "Bảng 2.",
            ...
            2, is_table=True)
    insert_table_here(sanitize_data_table(src_doc.tables[9]._tbl, 5, font_sz_pt=8.0))
    
    p = add_p()
    if is_en:
        add_styled_text(p, [
            "The implementation in this paper utilizes PyTorch 2.1 / Python 3.10, AdamW optimizer (lr = 10⁻³, weight decay = 10⁻⁴), "
            "batch size = 64, a maximum of 100 epochs with early stopping patience of 10. The experts have a hidden dimension of 128, "
            "and the router MLP has a hidden dimension of 64. Hardware environment: Intel Xeon Silver 4216 CPU, 512 GB RAM, and 4× NVIDIA Tesla T4 16 GB GPUs."
        ])
    else:
        add_styled_text(p, [
            "Cài đặt trong bài báo sử dụng PyTorch 2.1/Python 3.10, AdamW (lr=10⁻³, weight decay=10⁻⁴), batch size 64, tối đa 100 epoch với early stopping 10. Các expert có hidden size 128, router MLP 64. Phần cứng: Xeon Silver 4216, 512 GB RAM, 4×Tesla T4 16 GB."
        ])
    ...
    p_img2.add_run().add_picture('scratch/images/image2.png', width=Inches(3.45))
    ...
    set_cap("TABLE III." if is_en else "Bảng 3.",
            ...
            3, is_table=True)
    insert_table_here(sanitize_data_table(src_doc.tables[10]._tbl, 7, font_sz_pt=8.0))
    ...
    set_cap("TABLE IV." if is_en else "Bảng 4.",
            ...
            4, is_table=True)
    insert_table_here(sanitize_data_table(src_doc.tables[11]._tbl, 7, font_sz_pt=8.0))
    ...
    p_img3.add_run().add_picture('scratch/images/image3.png', width=Inches(3.45))
    ...
    set_cap("TABLE V." if is_en else "Bảng 5.",
            ...
            5, is_table=True)
    insert_table_here(sanitize_data_table(src_doc.tables[12]._tbl, 5, font_sz_pt=8.0))
    ...
    if is_en:
        add_styled_text(p, [
            ...
            "for gasoline/diesel. Router weight analysis in Fig. 4 indicates that under low GPR and short horizons, the CNN expert receives an average "
            "weight of approximately 0.48; when GPR exceeds the 90th percentile, the Wav-KAN weight surges from approximately 0.29 to 0.61 at medium–long horizons, "
            "while the CNN weight contracts to approximately 0.21. These findings demonstrate that the router executes genuine regime-switching rather than trivial output averaging."
        ])
    else:
        add_styled_text(p, [
            "Kết quả thực nghiệm ghi nhận rằng loại bỏ residual scaling làm MAE tăng khoảng 8,5%/6,3% ở H20 và 14,1%/11,8% ở H60 cho xăng/dầu. "
            "Phân tích router trong Hình 4 cho thấy ở GPR thấp và horizon ngắn, CNN có trọng số trung bình khoảng 0,48; khi GPR vượt phân vị 90%, "
            "trọng số Wav-KAN tăng từ khoảng 0,29 lên 0,61 ở horizon trung–dài và CNN giảm xuống khoảng 0,21. Các kết quả này cho thấy router thực sự "
            "thay đổi chế độ thay vì chỉ trung bình hóa đầu ra."
        ])
        
    p_img4 = add_p(align=WD_ALIGN_PARAGRAPH.CENTER, space_before=4, space_after=2)
    p_img4.add_run().add_picture('scratch/images/image4.png', width=Inches(3.45))
    ...
    if is_en:
        ...
        add_styled_text(p, [
            "Bài báo đề xuất GUMNetHet, một kiến trúc mixture-of-experts dị thể cho dự báo xác suất đa horizon giá xăng dầu thành phẩm. "
            "Mô hình kết hợp CNN-1D đa tỷ lệ, GRU-Attention và Wavelet-KAN thông qua bộ định tuyến nhận biết horizon, cùng đầu ra đa phân vị "
            "và residual scaling nhằm cải thiện độ ổn định khi dự báo xa."
        ])
    ...
    if is_en:
        ...
        add_styled_text(p, [
            "Trong tương lai, nghiên cứu sẽ tập trung vào đánh giá multi-seed, bổ sung probabilistic baselines, kiểm định thống kê cho dự báo multi-step "
            "và tích hợp biểu diễn tin tức địa chính trị theo thời gian thực."
        ])

    # --- REFERENCES ---
    p_ref_head = add_p(style_name='Heading 5', align=WD_ALIGN_PARAGRAPH.CENTER, space_before=8, space_after=4)
    p_ref_head.paragraph_format.keep_with_next = True
    r_rfh = p_ref_head.add_run("REFERENCES" if is_en else "TÀI LIỆU THAM KHẢO")
    r_rfh.font.name = "Times New Roman"
    r_rfh.font.size = Pt(10.0)
    ...
    body_elem.append(parse_xml(sect_final_xml))

    try:
        doc.save(out_path)
        print(f"Successfully generated: {out_path}!")
    except PermissionError:
        fallback_path = out_path.replace(".docx", "_updated.docx")
        doc.save(fallback_path)
        print(f"WARNING: '{out_path}' is open in Word. Saved to '{fallback_path}' instead.")

if __name__ == '__main__':
    build_faithful_ieee_manuscript(is_en=True, out_path="GUMNETHet_FAIRv4_final.docx")
    build_faithful_ieee_manuscript(is_en=True, out_path="GUMNETHet_FAIRv4_final_IEEE.docx")
    build_faithful_ieee_manuscript(is_en=False, out_path="GUMNETHet_FAIRv4_final_VI.docx")
