...
def style_heading_en(p, text, is_h1=True, font_size=10.0):
    p.text = ""
...
def style_caption_en(p, prefix, text, tag_num, is_table=True):
    p.text = ""
...
def add_styled_paragraph_en(p, segments, line_spacing=1.15, space_after=4, align=WD_ALIGN_PARAGRAPH.JUSTIFY):
    p.text = ""
    p.alignment = align
    ...
    for seg in segments:
        ...
        elif isinstance(seg, tuple) and seg[0] == 'bold':
            run = p.add_run(seg[1])
            run.font.name = 'Times New Roman'
            run.font.size = Pt(9.5)
            run.font.bold = True
...
def style_abstract_para_en(p, prefix, text_fragments):
    p.text = ""
...
def build_english_manuscript_complete(target_path):
    ...
    # --- 2. ABSTRACT & KEYWORDS ---
    style_abstract_para_en(p_abstract, "Abstract—", [
        ...
        "The 80% prediction interval attains ", ('bold', "PICP=82.4%"), " with ", ('bold', "PINAW=0.142"), ". Ablation studies and router "
        "analysis confirm that expert specialization and adaptive routing contribute substantially to model performance."
    ])
    
    style_abstract_para_en(p_keywords, "Keywords—", [
    ...
    style_heading_en(p_h1_intro, "1. INTRODUCTION", is_h1=True, font_size=10.0)
    ...
    add_styled_paragraph_en(p_intro_contrib, [
        ...
        ('bold', "(i) A heterogeneous MoE architecture "), "with feature partitioning grounded in economic principles and frequency domains; ",
        ('bold', "(ii) A horizon-aware routing mechanism "), "(horizon-aware routing); ",
        ('bold', "(iii) A multi-quantile prediction head "), "(multi-quantile head) combined with residual scaling to mitigate variance drift; and ",
        ('bold', "(iv) Extensive expanding walk-forward empirical evaluations "), "on ", ('m', 'N_val'), " trading days directly within "
    ...
    style_heading_en(p_h1_related, "2. RELATED WORK", is_h1=True, font_size=10.0)
    ...
    style_heading_en(p_h1_method, "3. PROPOSED METHODOLOGY: GUMNetHet", is_h1=True, font_size=10.0)
    style_heading_en(p_h2_prob, "3.1. Problem Formulation", is_h1=False, font_size=9.5)
    
    add_styled_paragraph_en(p_form1_txt, [
    ...
    add_styled_paragraph_en(p_form2_txt, [
    ...
    style_heading_en(p_h2_arch, "3.2. Overall System Architecture", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_arch_txt, [
    ...
    style_caption_en(p_cap_fig1, "Fig. 1.", "Architectural overview of GUMNetHet: feature partitioning, three heterogeneous experts, horizon-aware router, and multi-quantile head.", "1", is_table=False)
    
    style_heading_en(p_h2_cnn, "3.2.1. Price Momentum Expert: Multi-Scale 1D-CNN", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_cnn_txt, [
    ...
    style_heading_en(p_h2_gru, "3.2.2. Macroeconomic Regime Expert: GRU-Attention", is_h1=False, font_size=9.5)
    p_gru_txt.text = (
    ...
    style_heading_en(p_h2_wkan, "3.2.3. Non-Linear Shock-Absorption Expert: Wavelet-KAN", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_wkan_txt, [
    ...
    style_heading_en(p_h2_router, "3.2.4. Horizon-Aware Dynamic Gating Router", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_router_txt, [
    ...
    style_heading_en(p_h2_res, "3.2.5. Residual Scaling Bounding & Multi-Quantile Prediction Head", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_res_txt, [
    ...
    style_heading_en(p_h2_loss, "3.2.6. Dual-Loss Optimization", is_h1=False, font_size=9.5)
    p_loss_txt.text = (
    ...
    style_heading_en(p_h1_exp, "4. EXPERIMENTAL SETUP", is_h1=True, font_size=10.0)
    style_heading_en(p_h2_data, "4.1. Data and Walk-Forward Protocol", is_h1=False, font_size=9.5)
    
    add_styled_paragraph_en(p_data_txt, [
    ...
    add_styled_paragraph_en(p_data_wf_txt, [
    ...
    style_heading_en(p_h2_diag, "4.2. Econometric Diagnostics and Training Configuration", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_diag_txt, [
    ...
    style_caption_en(p_cap_tbl2, "Table 2.", "Condensed econometric statistical diagnostics for primary energy series.", "2", is_table=True)
    ...
    style_heading_en(p_h2_base, "4.3. Baseline Methods and Evaluation Metrics", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_base_txt, [
    ...
    style_heading_en(p_h1_res, "5. RESULTS AND DISCUSSION", is_h1=True, font_size=10.0)
    style_heading_en(p_h2_point, "5.1. Multi-Horizon Point Forecasting Performance", is_h1=False, font_size=9.5)
    
    add_styled_paragraph_en(p_point_txt, [
    ...
    style_caption_en(p_cap_fig2, "Fig. 2.", "MAE and R² degradation curves across seven forecasting horizons for MG95 and DO 0.001% (Seed=42).", "2", is_table=False)
    style_caption_en(p_cap_tbl3, "Table 3.", "Detailed empirical forecasting performance on MG95 gasoline (Seed=42). MAE/RMSE in USD/bbl; DA in percent.", "3", is_table=True)
    style_caption_en(p_cap_tbl4, "Table 4.", "Detailed empirical forecasting performance on DO 0.001% diesel (Seed=42).", "4", is_table=True)

    style_heading_en(p_h2_da, "5.2. Directional Accuracy Analysis", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_da_txt, [
    ...
    style_caption_en(p_cap_fig3, "Fig. 3.", "Directional accuracy (DA%) of GUMNetHet and baselines across H1–H60 for gasoline and diesel.", "3", is_table=False)

    style_heading_en(p_h2_prob_res, "5.3. Probabilistic Forecasting, Ablation, and Router Behavior", is_h1=False, font_size=9.5)
    add_styled_paragraph_en(p_prob_res_txt, [
        "The ", ('m', 'q_interval'), " prediction interval achieves an empirical coverage probability of ", ('bold', "PICP=82.4% "),
        "(exceeding nominal 80%) with a normalized average width of ", ('bold', "PINAW=0.142"), ". This validates that the model avoids under-coverage "
    ...
    style_caption_en(p_cap_tbl5, "Table 5.", "Condensed ablation study of GUMNetHet variants (Seed=42).", "5", is_table=True)
    
    p_res_scale_txt.text = (
    ...
    style_caption_en(p_cap_fig4, "Fig. 4.", "Top: Multi-quantile fan chart under high volatility. Bottom: Router gating weights across low-GPR and high-GPR regimes.", "4", is_table=False)

    # --- 8. SECTION 6: CONCLUSION & REFERENCES ---
    style_heading_en(p_h1_concl, "6. CONCLUSION", is_h1=True, font_size=10.0)
    ...
    add_styled_paragraph_en(p_concl2, [
    ...
    style_heading_en(p_h1_refs, "REFERENCES", is_h1=True, font_size=10.0)
    ...
    style_caption_en(new_caption_p, "Table 1.", "Expanding walk-forward train/test split configurations and empirical volatility characteristics across forecasting horizons.", "1", is_table=True)
    ...
    print(f"Successfully generated complete English manuscript at {target_path}!")

if __name__ == '__main__':
    target = sys.argv[1] if len(sys.argv) > 1 else 'GUMNETHet_FAIRv3_EN.docx'
    build_english_manuscript_complete(target)
