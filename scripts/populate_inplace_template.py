...
def populate_inplace_template(is_en=True, out_path="GUMNETHet_FAIRv4_final.docx"):
    ...
    # Fix Section 0 top margin to prevent title clipping (set top margin to 36pt = 0.5 in)
    doc.sections[0].top_margin = Pt(36)
    
    # 1. P00: Title
    p0 = doc.paragraphs[0]
    p0.text = ""
    p0.style = 'paper title'
    p0.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_t = p0.add_run("Robust Probabilistic Energy Forecasting under Geopolitical Shocks: An Adaptive Mixture of Local-Global Experts")
    ...
    # 3. Authors: P04, P05, P06
    def fill_author(p, num_str, sup_str, name, dept, org, city, email):
        p.text = ""
        p.style = 'Author'
        ...
        # Line 4: City
        r_city = p.add_run(city + "\n")
        r_city.font.name = "Times New Roman"
        r_city.font.size = Pt(9.0)
        
        # Line 5: Email
        r_mail = p.add_run(email)
        r_mail.font.name = "Times New Roman"
        r_mail.font.size = Pt(9.0)

    if is_en:
        fill_author(doc.paragraphs[4], "1", "st", "Phuoc Anh Dung Nguyen", 
                    "Faculty of Information Technology", 
                    "Ho Chi Minh City University of Technology (HUTECH)", 
                    "Ho Chi Minh City, Vietnam", 
                    "anhdungnguyen955@gmail.com")
        fill_author(doc.paragraphs[5], "2", "nd", "Danh Huong Bui*", 
                    "Faculty of Information Technology", 
                    "Ho Chi Minh City University of Technology (HUTECH)", 
                    "Ho Chi Minh City, Vietnam", 
                    "bd.huong@hutech.edu.vn")
        fill_author(doc.paragraphs[6], "3", "rd", "Van Quy Hoang", 
                    "Faculty of Information Technology", 
                    "Thuyloi University (TLU)", 
                    "Hanoi, Vietnam", 
                    "hoangvanquy@tlu.edu.vn")
    else:
        fill_author(doc.paragraphs[4], "1", "st", "Nguyễn Phước Anh Dũng", 
                    "Khoa Công nghệ Thông tin", 
                    "Trường Đại học Công nghệ TP.HCM (HUTECH)", 
                    "TP. Hồ Chí Minh, Việt Nam", 
                    "anhdungnguyen955@gmail.com")
        fill_author(doc.paragraphs[5], "2", "nd", "Bùi Danh Hường*", 
                    "Khoa Công nghệ Thông tin", 
                    "Trường Đại học Công nghệ TP.HCM (HUTECH)", 
                    "TP. Hồ Chí Minh, Việt Nam", 
                    "bd.huong@hutech.edu.vn")
        fill_author(doc.paragraphs[6], "3", "rd", "Hoàng Văn Quý", 
                    "Khoa Công nghệ Thông tin", 
                    "Trường Đại học Thủy lợi (TLU)", 
                    "Hà Nội, Việt Nam", 
                    "hoangvanquy@tlu.edu.vn")

    doc.paragraphs[7].text = "" # Clear row 2

    # Remove all paragraphs starting from index 10 in the template
    body_elem = doc._body._body
    
    # Save the final sectPr of the template (Section 4 or Section 3 2-column continuous break)
    # Let's inspect where Section 3 starts (it is at P08 or P09)
    # All paragraphs from P10 downwards are template sample text
    p10_elem = doc.paragraphs[10]._p
    
    # Remove all subsequent siblings after P09
    curr = p10_elem
    to_remove = []
    while curr is not None:
        next_sibling = curr.getnext()
        to_remove.append(curr)
        curr = next_sibling
        
    for elem in to_remove:
        # if it's a sectPr at the very end of body, keep it!
        if elem.tag.endswith('sectPr') and elem == body_elem.xpath('./w:sectPr')[-1]:
            continue
...
if __name__ == '__main__':
    populate_inplace_template(is_en=True, out_path="GUMNETHet_FAIRv4_final.docx")
    populate_inplace_template(is_en=False, out_path="GUMNETHet_FAIRv4_final_VI.docx")
