...
def set_clean_p_text(p, new_text):
    # Keep only w:pPr if it exists
...
# Polished text dictionary
paragraphs_to_polish = [
    # 1. Abstract
    (
    ...
    # 2. Intro Para 1
    (
        'The refined petroleum supply in emerging Asian markets',
    ...
    # 3. Intro Para 2
    (
        'Recent deep learning models—including patch-based Transformers',
    ...
    (
        'Forecasted spot price levels are subsequently recovered through the exact inverse exponential transformation',
    ...
    # 6. Methodology Wavelet-KAN
    (
        'To model non-linear structural shifts and localized tail shocks triggered by geopolitical events',
    ...
    # 7. Methodology Residual scaling
    (
        'At extended forecast horizons (such as H60), unconstrained neural outputs risk compounding variance drift',
    ...
    (
        'To prevent look-ahead bias, all predictor series are indexed at t - 1',
    ...
    (
        'Fig. 2 alongside Table III and Table IV show that GUMNetHet attains lower MAE than all reported baselines',
    ...
    (
        'Directional accuracy in Fig. 3 shows distinct behavior between short and long horizons',
    ...
    (
        'The [q0.1, q0.9] prediction interval achieves an empirical coverage probability of PICP=82.4%',
    ...
    # 12. Conclusion
    (
        'This paper proposed GUMNetHet, a heterogeneous mixture-of-experts architecture for multi-horizon probabilistic forecasting of refined petroleum product prices subject to geopolitical shocks',
...
for p in root.iter(f'{{{W_NS}}}p'):
    p_text = get_p_text(p)
    for trigger, new_text in paragraphs_to_polish:
        if trigger in p_text:
            set_clean_p_text(p, new_text)
            print(f"✓ Cleaned and updated: '{trigger[:45]}...'")
            break
