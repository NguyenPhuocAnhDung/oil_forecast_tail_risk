...
# Define list of (regex_pattern, clean_replacement_text)
replacements = [
    # 1. Abstract
    (
    ...
    # 3. Intro Para 2
    (
        r'Recent deep learning models—including patch-based Transformers.*?'
        r'often leading to redundant representations and expert collapse\.',
    ...
    (
        r'To overcome these challenges, we introduce GUMNetHet \(Heterogeneous Gated Unified Mixture Network\).*?'
        r'All source code, model checkpoints, and evaluation scripts are publicly available at: https://github\.com/NguyenPhuocAnhDung/oil_forecast_tail_risk\.',
    ...
    # 6. Point forecast discussion
    (
        r'Fig\. 2 alongside Table III and Table IV show that GUMNetHet attains lower MAE than all reported baselines for both products across all horizons.*?rather than precise long-range trajectory forecasting\.',
    ...
    # 7. Conclusion
    (
        r'This paper proposed GUMNetHet, a heterogeneous mixture-of-experts architecture for multi-horizon probabilistic refined oil price forecasting\..*?rather than in long-range directional trajectory prediction\.',
...
for pat, repl in replacements:
    # Use re.sub with lambda to avoid escape issues
    new_xml, cnt = re.subn(pat, lambda m: repl, doc_xml, flags=re.DOTALL)
    if cnt > 0:
        doc_xml = new_xml
        count_applied += 1
        print(f"✓ Applied rewrite {count_applied}")
    else:
        print(f"⚠️ Pattern not found: {pat[:60]}...")
...
print(f"\n🎉 Successfully saved polished document to {docx_out} ({count_applied} blocks updated)!")
